...
def register():
    ...
    if form.validate_on_submit():
        ...
        db.session.add(user)
        db.session.commit()
        flash(f'Your account has been created! You are new able to log in', 'success')
...
def save_picture(form_picture):
    ...
    #Resize image before being saved
    output_size = (125,125)
...
def account():
    ...
    #Get current user image profile for deleting porpouse in case of image update
    old_account_picture = current_user.image_file
    if form.validate_on_submit():
